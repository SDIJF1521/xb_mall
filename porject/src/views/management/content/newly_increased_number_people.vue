...
export default {
  name: 'NewlyIncreasedNumberPeople',
  setup() {
    ...
    const sendRequest = async () => {
      try {
        ...
        await axios.post(`http://127.0.0.1:8000/api/today_user_list`,formdata)
        .then(ref =>{
            if (ref.status === 200) {
                if (ref.data.current){
                    number_people.value=ref.data.user_list.length;
                }else{
...
.el-card {
  padding: 22px;
  border-radius: 12px;
  box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05), inset 0 -1px 3px rgba(255, 255, 255, 0.8);
