...
- 📬 **客服消息 HTTP 接口** - `GET /api/cs_user_sessions` 用户会话列表、`GET /api/cs_user_history` 消息历史、`GET /api/cs_unread_count` 未读数、`POST /api/cs_mark_read` 标记已读；用户发消息时卖家未读 +1，卖家回复时用户未读 +1，打开会话时自动清零
