...
from data.data_mods import PayConfigSave
...
PAY_CONFIG_COLLECTION = "pay_config"


# ────────────────── 保存 / 更新支付配置 ──────────────────

@router.post("/manage_pay_confing")
async def save_pay_config(
    data: PayConfigSave,
    ...
    """保存平台支付配置（公钥模式，全局唯一文档）。"""
    try:
        ok, msg, _ = await verify_admin_with_permission(
            db, redis, access_token, required="admin.pay_config"
        )
        if not ok:
        ...
        existing = await mongo.find_one(PAY_CONFIG_COLLECTION, {"is_active": True})
        if existing:
            await mongo.update_one(
                PAY_CONFIG_COLLECTION,
                {"_id": ObjectId(existing["_id"])},
                {"$set": doc},
            )
        else:
            doc["created_at"] = datetime.now()
            await mongo.insert_one(PAY_CONFIG_COLLECTION, doc)
...
async def get_pay_config(
    ...
    try:
        ...
        doc = await mongo.find_one(PAY_CONFIG_COLLECTION, {"is_active": True})
        if not doc:
...
@router.post("/manage_pay_confing/verify")
async def verify_pay_config(
    access_token: str = Header(...),
    db: aiomysql.Connection = Depends(get_db),
    redis: RedisClient = Depends(get_redis),
    mongo: MongoDBClient = Depends(get_mongodb_client),
):
    """校验密钥格式并向支付宝网关发起连通性测试。"""
    try:
        ok, msg, _ = await verify_admin_with_permission(
            db, redis, access_token, required="admin.pay_config"
        )
